...
int main() 
{
    ...
    double time_elapsed = 50;         //total time passed during simulation
    ...
    int num_bodies = 2;               //number of bodies in the system
    int skip_save = 10;                //the number of loops that need to pass before saving to a txt file
    ...
    for (int counter = 0; counter < num_bodies; counter++)
    {
        ...
        init_x_pos[counter] = 10 * r * sin(phi) * cos(theta);//getPoint(0)*5;//cos(theta) * sqrt(1 - u*u); //normalRandom()*sigma+Mi;//returnRandoms(1, 50);
        init_y_pos[counter] = 10 * r * sin(phi) * sin(theta);//getPoint(1)*5;//sin(theta) * sqrt(1 - u*u);//normalRandom()*sigma+Mi;//returnRandoms(1, 50);
        init_z_pos[counter] = 10 * r * cos(phi); //getPoint(2)*5;//u;//normalRandom()*sigma+Mi;//returnRandoms(1, 50);
        ...
        init_z_veloc[counter] = 0;//drand(-.1,.1);//normalRandom()*sigma+Mi;//returnRandoms(1, 50);
    }
